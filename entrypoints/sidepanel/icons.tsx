// 轻量内联 SVG 图标（Lucide 风格，stroke + currentColor）。
// 取代按钮里的 emoji，保证一致性与可访问性（ref: ui-ux-pro-max「no-emoji-icons」）。
...
// —— 活动时间线的工具类别图标（见 lib/agent/activity-category.ts）——
// 它们标的是"这一步在做哪类事"，不是成败；状态只靠颜色和失败时的 ✗ 表达。
...
export function IconPointer({ className }: IconProps) {
  return (
    <Svg className={className}>
      <path d="M5 3l6.5 17 2.2-7.3 7.3-2.2z" />
    </Svg>
  );
}
