...
- ⚡ **快捷指令**：内置「总结本页 / 翻译划词 / 帮我填表 / 润色改写 / 专注阅读」五条，可编辑、删除、恢复默认，也可新增自定义指令；每条指令有明确的上下文作用域（当前页面 / 已选文本 / 不使用网页上下文），作用域同时决定它能不能动页面。输入框输入 `/` 即可唤出。`页面`作用域的指令会把正文随第一轮一起带上，省掉一次额外的读页往返
