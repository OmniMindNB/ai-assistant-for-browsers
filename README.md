...
- 🔒 **确认后才动手**：每轮第一次写操作执行前，Runi 会请求确认，并将决定仅在该轮内复用——Deny-First 权限模型 + 注入脚本静态扫描（AST 危险 API 检测）+ SSRF 防护
...
- 📎 **本地文件上下文**：可附加文本、图片或 PDF。PDF 文字在本地提取（20 MB / 60,000 个字符，不含 OCR），仅用于当前一轮，历史中只保留文件元数据。
