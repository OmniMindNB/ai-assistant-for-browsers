...
- **`entrypoints/sidepanel/`** (React app) — the chat UI. `store.ts` (Zustand) owns chat/session state and drives the agent; `App.tsx` renders messages, tool-call state, and the confirmation card.
