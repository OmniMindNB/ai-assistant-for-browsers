...
- **`agent-overlay.ts`** / **`tab-overlay-state.ts`** — the execution-period overlay: a purely visual, `pointer-events: none` signal placed over the page once a write tool is approved, so the user can see an action happening without it ever blocking their own input. `agent-overlay.ts` is the content-script-side DOM code (animations use `element.animate()`, never `<style>`/`@keyframes`, since the strict-CSP sites most worth auto-filling — banking, government — block injected `<style>` tags). `tab-overlay-state.ts` is the background-side per-tab overlay state in `browser.storage.session`; because `storage.session` isn't readable from content scripts, a post-navigation overlay rebuild is driven by `background.ts`'s `tabs.onUpdated` listener re-reading this table, not by the content script self-checking.
