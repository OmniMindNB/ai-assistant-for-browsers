...
- **`permissions.ts`** — Deny-First policy: `decideToolPermission` classifies every tool into `always_allow` (read-only) / `auto_allow` (all known writes) / `confirm_always` (structurally detected form submissions — asked every time, never served from the turn cache) / `deny` (unknown tools, plus tool-specific hard blocks — currently non-http(s) navigation targets and everything in `DENY_TOOL_NAMES`). `READ_ONLY_TOOL_NAMES`/`AUTO_APPROVE_TOOL_NAMES` are the single source of truth for tool tiering; `WRITE_TOOL_NAMES` drives the system prompt, write budget, and overlay.
