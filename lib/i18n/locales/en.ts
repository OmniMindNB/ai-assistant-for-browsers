...
export const en: Record<keyof typeof zh, string> = {
  ...
  'provider.visionModelsHint': 'Check the models that accept image input. The screenshot tool is only enabled for checked models — sending an image to a model that cannot accept one fails outright. Screenshots are sent as images and are not covered by the content-redaction rules.',
  'provider.visionModelsEmpty': 'Fill in the model above first',
  ...
  'sidebar.clearAllHistory': 'Clear all',
  'sidebar.confirmClearAllHistory': 'Clear all?',
  'sidebar.clearAllHistoryAriaLabel': 'Clear all conversation history',
  'sidebar.confirmClearAllHistoryAriaLabel': 'Confirm clear all conversation history? Click again to delete everything.',
  ...
  'agentActivity.done.openTabRedirected': 'Opened new tab "{target}" (redirected from "{requested}")',
  ...
  'agentActivity.done.clickPartial': 'Clicked {ok} of {total} targets',
  ...
  'agentActivity.done.navigateRedirected': 'Navigated to "{target}" (redirected from "{requested}")',
  ...
  'store.shortcutPageWindowedNoOutlinePrompt':
    '{instruction}\n\nThe current page (title: {title}, url: {url}) has a body of {total} characters, which exceeds what fits in one turn. What follows is the beginning and the end of the same page, not two different pages.\n\nBeginning (characters 0–{headEnd}, JSON string):\n{head}\n\nEnd (characters {tailStart}–{total}, JSON string):\n{tail}\n\n{omitted} characters in the middle were omitted, and this page has no usable heading structure. When you need detail from the middle, call browser_find_text with keywords from the task itself to jump straight to it. Do not page through with browser_read_page from the start: the text above is not compacted into a summary, but context has a total size budget, and calling a read tool again can push it out of context entirely and permanently along with this turn\'s question, so you would lose the beginning to gain the end.',
  ...
  'recordedTask.privacyNote': 'This stays on this device; it is sent to your configured model to summarize a reusable method and when you run it.',
  ...
  'recordedTask.summaryTruncated': 'the model reply was cut off before it finished (a reasoning model may have used up its output budget)',
  'recordedTask.playbookIncomplete': 'Works on and at least one method step must be filled in.',
