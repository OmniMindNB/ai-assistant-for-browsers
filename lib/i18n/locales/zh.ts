...
export const zh = {
  ...
  'settings.navBudget': '任务预算',
  'budget.heading': '工具调用预算',
  'budget.description': '每条消息允许 Agent 调用多少次工具。撞到上限时它会立刻基于已有结果收尾。纯等待和最初几次失败的调用不计入预算。',
  'budget.standard': '标准',
  'budget.generous': '宽松',
  'budget.detail': '读取 {read} 次，开始写入后再追加 {write} 次',
  'budget.generousHint': '长表单、跨页面流程这类本来就要几十步的任务更少被中途打断；代价是任务跑偏时也会跑得更久。',
  ...
  'provider.visionModelsHint': '勾选能接收图片输入的模型。只有勾选的模型才会启用截图工具——给不支持图片的模型发图片会直接报错。截图以图片形式发送，不会经过"内容脱敏"规则处理。',
  'provider.visionModelsEmpty': '先填写上面的模型',
  ...
  'sidebar.clearAllHistory': '清空全部',
  'sidebar.confirmClearAllHistory': '确认清空？',
  'sidebar.clearAllHistoryAriaLabel': '清空全部历史会话',
  ...
  // 步骤右侧的结果计数
  'agentActivity.note.results': '{count} 条结果',
  'agentActivity.note.resultsTruncated': '{count}+ 条结果',
  'agentActivity.note.fields': '{count} 个字段',
  'agentActivity.note.tabs': '{count} 个标签页',
  'agentActivity.note.filled': '写入 {ok}/{total}',
  // 折叠头：类别名与摘要句
  'agentActivity.category.read': '读取页面',
  'agentActivity.category.screenshot': '截图',
  'agentActivity.category.write': '修改页面',
  'agentActivity.category.interact': '操作控件',
  'agentActivity.category.navigate': '页面跳转',
  'agentActivity.category.wait': '等待页面',
  'agentActivity.category.ask': '询问你',
  'agentActivity.category.report': '汇报结果',
  'agentActivity.summary.line': '{categories}，共 {count} 步',
  'agentActivity.summary.countOnly': '共 {count} 步',
  'agentActivity.summary.running': '正在执行，已走 {count} 步',
  'agentActivity.summary.separator': '、',
  'agentActivity.summary.expand': '展开执行步骤',
  'agentActivity.summary.collapse': '收起执行步骤',
  ...
  'agentActivity.tool.goBack': '后退到上一页',
  ...
  'agentActivity.failed.findText': '查找文字 "{target}" 失败',
  ...
  'shortcut.builtinFocusReadName': '专注阅读',
  'shortcut.builtinFocusReadPrompt':
    '请把当前网页整理成适合专注阅读的样子：先找出真正的正文容器，以及广告位、悬浮条、Cookie 提示、订阅弹窗、侧边推荐、吸底工具条这类干扰元素，然后隐藏干扰元素，并给正文放大字号、加大行高、收窄行宽。注意不要隐藏正文本身、主导航和登录状态相关的元素——宁可少隐藏，也不要误伤。用尽量少的调用完成：把同类干扰元素合并成一个选择器一次处理，不要一个一个改。做完后用一句话说明隐藏了什么、调整了什么，不要罗列 CSS。请使用中文回答。',
  'shortcut.builtinFillFormName': '帮我填表',
  'shortcut.builtinFillFormPrompt':
    '请帮我填写当前页面上的表单：先读出所有字段，判断这是什么表单（注册、登记、报名、下单、申请等）。能从页面上下文或我之前提供过的信息推断出来的直接填；推断不出来的必填字段，一次性把缺的信息全部问清楚，不要一个字段问一次。密码和支付卡号这类敏感字段一律不填，直接告诉我需要我自己输入。填完后核对写入结果，用一句话汇报填了哪些、跳过了哪些。除非我明确要求，否则不要提交表单。请使用中文回答。',
  ...
  'shortcut.builtinPolishName': '润色改写',
  'shortcut.builtinPolishPrompt':
    '请润色改写选中的文本：先判断它的体裁与场合（邮件、评论、社交帖子、提交信息、正式文书等），据此调整语气与措辞。保持原意、原语种和原有格式（换行、列表、代码、链接原样保留），不要增删事实，不要拔高承诺。目标是更清楚、更自然、更简洁，去掉冗词和翻译腔，而不是堆砌辞藻。只输出改写后的文本本身，不要加"改写如下"之类的开场白，也不要解释改了什么；如果原文已经足够好，就原样输出。',
  ...
  'shortcut.save': '保存',
  'shortcut.confirmDeleteAria': '确认删除快捷方式 {name}？再点一次即删除。',
  'shortcut.confirmRestoreTitle': '恢复预设快捷方式？',
  'shortcut.confirmRestore': '会清空你自建的快捷方式，并把改过的内建文案还原为默认。此操作无法撤销。',
  'shortcut.moveUp': '上移',
  ...
  'shortcut.confirmRepairInvalidTitle': '删除无效的快捷方式？',
  'shortcut.confirmRepairInvalid': '无效条目会被移除，有效条目保留。此操作无法撤销。',
