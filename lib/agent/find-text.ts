...
//
// find-text-dom.ts 的注入函数因 executeScript 的序列化约束 see top comment 无法在
...
// 把这一轮 find_text 命中并入现有句柄表：保留 f* s* 发放的句柄，
// 只替换上一轮 find_text 自己发放的 t*——这一轮的命中集合已经变了，旧的不该继续被信任，
// 与 browser_get_form 每次整表覆写是同一个理由，参看设计文档 section 4.4。
//
// 换了页面 existing.url 与 currentUrl 不符时，连 f* s* 也不保留：
// 它们本就对着别的页面，硬并入只会在下次写入时统一因 url 不符判 stale，保留没有意义。
export function mergeFindTextHandles(
  ...
  hits.forEach((hit, index) => {
    fields[`t${index + 1}`] = {
      ...
      // 旁人读句柄表时语义上说得通，参看 lib/agent/fill-form-request.ts 的
      // planFieldClick / planFieldScroll。
      kind: 'button',
      // 与 snapshotFields 同一惯例：句柄表里的 frameOrigin 无论主/子帧都完整保存，
      // 是否在写入时转发由 resolveExpectOrigin isChildFrameHandle 决定，
      // 不在这里过滤，参看 fill-form-request.ts 2026-09-05 final review Important 1。
      frameId: hit.frameId,
