...
describe('turn-snapshot', () => {
  ...
  it('captures html/body attribute snapshots so page-wide style/class changes (e.g. eye-care mode) can be restored', async () => {
    const snapshot = await beginSnapshotIfNeeded(TAB_ID, {
      url: 'https://a.example',
      headHTML: '',
      bodyHTML: '',
      htmlAttrs: [['lang', 'en']],
    ...
    expect(snapshot.htmlAttrs).toEqual([['lang', 'en']]);
    expect(snapshot.bodyAttrs).toEqual([['class', 'theme-light']]);
  });

  it('records a storage entry only once per key', async () => {
    await beginSnapshotIfNeeded(TAB_ID, {
      url: 'https://a.example',
      headHTML: '',
      bodyHTML: '',
      htmlAttrs: [],
