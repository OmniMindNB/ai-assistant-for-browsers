...
Runi 采用本地优先设计：Provider 设置、API Key、同意状态和对话历史保存在你的浏览器中。Runi 不运营开发者后端，不接入分析或广告 SDK。当你使用 AI 功能时，API Key、当前提示词、近期对话上下文和与当前页面有关的工具结果会由扩展直接发送到你配置的 AI Provider 端点，并由该 Provider 按其自身条款和隐私政策处理。
