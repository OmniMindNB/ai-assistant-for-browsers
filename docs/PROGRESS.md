...
| 日期 | 内容 | 关联 |
|------|------|------|
| 2026-07-29 | 自定义快捷方式：新增独立的本地快捷方式存储；两个内置项支持编辑、删除和恢复默认；明确“当前页面 / 已选文本 / 不使用网页上下文”三种强作用域边界；桌面设置页新增快捷方式导航，侧边栏设置复用紧凑版管理界面；输入区按存储顺序直显前三项，其余收进“更多”；移除空状态中央快捷卡片。代码层已通过 210/210 单测、TypeScript 类型检查和 Chrome MV3 生产构建；未打包扩展的真机交互验收待执行。 | [设计](superpowers/specs/2026-07-29-custom-shortcuts-design.md) |
