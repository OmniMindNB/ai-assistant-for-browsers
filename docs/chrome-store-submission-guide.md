...
本指南用于更新**现有的 Runi Chrome Web Store 商品**，版本迁移为 `1.0.0 → 1.1.0`。必须继续使用现有商品 ID，不要新建另一个商品。
...
- 产物为 `.output/runi-1.1.0-chrome.zip`。
...
3. 在该商品的 Package 页面上传 `.output/runi-1.1.0-chrome.zip`。
...
6. 不得因为 Runi 没有开发者后端就填写“完全不收集数据”。
