# Chrome Web Store `1.4.0` 更新操作指南

最后更新：2026-09-21

本指南用于更新**现有的 Runi Chrome Web Store 商品**到 `1.4.0`。必须继续使用现有商品 ID，不要新建另一个商品。
...
- 产物为 `.output/runi-1.4.0-chrome.zip`。
- 产物 `manifest.json` 中 `version` 为 `1.4.0`，`default_locale` 为 `zh_CN`。
...
3. 在该商品的 Package 页面上传 `.output/runi-1.4.0-chrome.zip`。
