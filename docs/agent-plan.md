...
> 当前依赖状态：`@earendil-works/pi-agent-core` 保持在 0.79.3。仓库管理的 pnpm patch 仅通过其公开 `Agent` 包装器转发 `prepareNextTurnWithContext` 与 `shouldStopAfterTurn`，以支持上述最终回答回合与强制结束语义；升级该依赖时必须重新评估此 patch。
