...
## Addendum 2: plan gap found during Task 12 live QA

Live QA with a real LLM (DeepSeek `deepseek-v4-pro` via an OpenAI-compatible endpoint) found that for a page-modification request, the model never attempted to call any write tool. It narrated intent repeatedly in prose ("现在我来注入...") and then falsely claimed it had "used up its tool budget" or "couldn't execute JavaScript" — neither was true — and fell back to suggesting the user paste code into devtools manually. The "Agent 工具调用" activity panel confirmed zero tool calls fired the entire turn.

Root cause: `SYSTEM_PROMPT` in `entrypoints/sidepanel/store.ts` (untouched by any of the 12 tasks — it wasn't in scope of Spec-0001, which is itself part of the gap) was entirely framed around read-only page analysis and never mentioned that the write tools this plan added exist, or that the model should call them directly rather than investigate extensively first. This was a plan-authorship gap, not an implementer defect — no task's brief ever asked anyone to update the system prompt.

An earlier symptom during the same QA session (the model defaulting to a different, already-configured provider — `deepseek-chat` — that appeared not to support tool-calling at all, not even for pre-existing read-only tools) was isolated as a separate, pre-existing environment/provider issue unrelated to this plan: switching to a tool-calling-capable model (`deepseek-v4-pro`) immediately fixed it, confirmed by a controlled A/B test that reverted the prompt fix and reproduced the same zero-tool-calls failure on `deepseek-chat` alone, ruling out the prompt fix itself as the cause.

Fixed directly in `entrypoints/sidepanel/store.ts`'s `SYSTEM_PROMPT`: added two sentences after the existing opening sentence, before the pre-existing implementation-analysis guidance — one listing all 10 write/interaction tool names, one instructing the model to call them directly for modification requests (skipping the full inspection pass meant for analysis questions), noting the one-time-per-turn confirmation gate, and explicitly telling it not to bypass the tools to suggest manual workarounds. Verified live end-to-end after the fix: confirmation card fired once, approval auto-ran the remaining 5 write calls in the same turn with no further prompts, the page visually changed as requested, undo correctly reverted it, denying a confirmation was handled gracefully with no retry loop, and a fresh turn after an approved one still required a new confirmation.
