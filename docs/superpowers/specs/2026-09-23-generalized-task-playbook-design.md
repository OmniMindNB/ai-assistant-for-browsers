...
- 总结不阻塞保存：`loading` 时也能直接保存录制步骤版本（存下的没有 `playbook`）。`completeOnce` 没有超时，若此时禁用保存，模型卡住时用户只能关抽屉丢掉编辑。但这是降级保存，不能悄悄发生：`loading` 期间保存按钮降为次要样式，文案改为"跳过整理，直接保存"，并附一行说明"只含录制步骤，不含通用做法"；`ready`/`failed` 后恢复为普通"保存"。
