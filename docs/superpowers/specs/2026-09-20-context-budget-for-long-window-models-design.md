...
2026-09-20 联网核实（来源见文末）。支持范围经三轮收窄：本地模型（26e9293）、用户判定过期的型号（`glm-4.7`、`glm-4.7-flash`、`kimi-k2.6`、`kimi-k2.7-code`），最后是智谱 GLM 与 Moonshot 整家移除——两家删掉过期型号后各自只剩一个在售型号，`models` 列表形同虚设，与其维护两个单模型预设不如让用户走自定义端点。剩余预设：
