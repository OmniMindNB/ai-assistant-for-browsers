...
2026-09-20 联网核实（来源见文末）。已排除本地模型（26e9293）与用户判定过期的型号（`glm-4.7`、`glm-4.7-flash`、`kimi-k2.6`、`kimi-k2.7-code`）：

| Provider | 模型 | 上下文窗口 |
|---|---|---|
| DeepSeek | `deepseek-v4-pro` / `deepseek-v4-flash` | 1,048,576 |
| OpenAI | `gpt-5.6-sol` / `terra` / `luna` | 1,050,000 |
| 通义千问 | `qwen3.7-max` / `qwen3.6-flash` | 1,000,000 |
| | `qwen3.7-plus` | 未单独确认（同系宣称 1M） |
| 智谱 GLM | `glm-5.2` | 1,048,576 |
| Moonshot | `kimi-k3` | 1,048,576 |
| Anthropic（无预设，走 `anthropic-messages`） | `claude-opus-5` / `claude-sonnet-5` | 1,000,000 |

**下限 1,000,000。没有任何一个还停在 128k。** 写死的 128000 不只是保守，它对所有支持的模型都是错的，而 `pi-agent-core` 拿这个数做自己的窗口管理。
...
这里有一个**真实存在但被接受的不对称**：预取正文进的是 user 消息，而 user 消息永远不会被摘要压缩（`context-budget.ts:60`）。一份 200,000 字符的预取会永久钉在上下文里，而同样大小的工具结果下一轮就被压成一行。设计过程中提出过三条路——跟着涨 / 预取单设更低常量 / 让预取正文可被压缩——用户选定第一条。
...
**一、中转端点跑老模型会撞供应商 400。** 今天 `MAX_CONTEXT_CHARS = 90000` 顺带保护了窗口 128K 的端点；抬到 350,000 之后不再保护。而这道字符闸门当初存在的理由恰恰是"不要让失败发生在服务端、用户等完一整轮却拿不到任何回答"（`context-budget.ts:71`）。这是用一种失效换另一种，按用户"不考虑老模型"的判断接受。

**二、一份最大 200,000 字符的预取正文永久占据上下文。** 见 §8，已接受。

**三、`qwen3.7-plus` 的窗口未单独确认。** 即使它实际是 256K 而非 1M，本设计的最坏情况 391,000 token 仍然超出——这是唯一一个数字上不安全的已知缺口。实施时应单独核实，或从预设 `models` 中移除。
...
1. 预设清理：删 `glm-4.7`、`glm-4.7-flash`、`kimi-k2.6`、`kimi-k2.7-code`。**阻塞项**：GLM 预设将只剩 `glm-5.2`、Moonshot 只剩 `kimi-k3`，在售同系型号名需用户提供，不得臆造。
