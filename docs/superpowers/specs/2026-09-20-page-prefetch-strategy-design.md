...
三个分支都跑在**脱敏之后**的正文上：`extractActivePage` 在返回给面板之前已经过了 `redactText`，所以长度判断和头尾切分用的都是模型最终会看到的那份文本，不存在"按原文切、按脱敏后发"的错位。骨架只在 `windowed` 分支进 prompt——`full` 分支的正文里标题本来就在。
...
- **上下文预算（2026-09-20 复审更正）**：`full` 分支最大 48000 字符的 user turn，叠加一次满额 `browser_get_html`（同样 48000）就是 96000 > `MAX_CONTEXT_CHARS`（90000），会触发重切。这里最初的论证是错的——"`compactAgentMessages` 永远保留最后一条消息"和"预取正文进的是 user turn，不会被压成一行摘要"两句话本身都对，但都保护不到预取正文：`recutStartForCharBudget`（`lib/agent/agent.ts` 约 632-643 行）是从**最新一条往回**累加到 `CONTEXT_RECUT_TARGET_CHARS`（60000），而预取正文是窗口里**最旧**的那条 user 消息，所以它是第一个被切掉的，不是被保护的最后一条；且 `planContextWindow` 只会让 `state.start` 前进，切掉之后整场对话都回不来——不是"压成摘要"，是直接从窗口里消失。量化对比：改动之前预取上限是 24000，加一次满额只读工具是 24000 + 48000 = 72000，不会触发重切；改动之后是 48000 + 48000 = 96000，会触发。
  控制端裁定：**本次不收缩头尾长度，也不收缩只读工具的默认上限**——总结类任务通常整场只有预取这一轮，不会叠加满额 DOM 读取，为不常见的组合收缩头尾会牺牲更常见的路径。后续可选项：让只读工具在本轮已经携带预取正文时自动降低默认 `maxChars` 上限（而不是双方各自不知情地都拿满额）。
