...
| 可见标签页截图 | 仅在截图工具被调用时，截取目标活动标签页的可见区域 | 在本次任务的运行上下文中缩放并重编码为 JPEG，不写入 Runi 的持久化对话数据库 | 只有在你声明所选模型支持图片输入时，截图工具才会提供给模型；调用后，截图会作为图片输入直接发送到你配置的 AI Provider。截图是像素，本节所述的文本脱敏规则对截图不生效，截图时屏幕上可见的内容都可能到达 Provider |
...
Runi 仅为用户请求的核心功能处理数据：理解当前页面和用户选择的文件、回答有依据的问题、分析页面实现方式，以及执行页面操作。已知页面操作会自动执行；检测到的表单提交每次都需要确认。
...
当你发起 Agent 请求时，即表示你指示 Runi 根据完成该请求的需要，将当前提示词、近期对话上下文、API Key、相关页面结果，以及你为该请求选择的文件内容发送到你配置的 AI Provider。发起请求后，只读工具和已知页面操作可以运行；检测到的表单提交每次执行前都会请求确认。
...
- 临时的标签页与对话对应状态、多标签页操作目标和执行遮罩状态保存在 `chrome.storage.session`。
- 页面工具结果和截图图片不会写入 Runi 的持久化对话数据库。
...
| `tabs` | 识别并校验目标标签页、读取其标题与 URL、打开扩展设置页，执行用户请求的导航，以及作为已知操作自动打开/关闭/切换 Runi 自己打开过的标签页 |
| `scripting` | 在目标页面运行随扩展打包的页面读取与结构化写入函数 |
| `storage` | 保存 Provider 设置、API Key、快捷方式、语言、主题和工作台偏好，以及临时的标签页与对话对应状态、多标签页操作目标和执行遮罩状态 |
| `sidePanel` | 承载 Runi 的主要界面 |
| `alarms` | 在用户发起的 Agent 任务运行期间维持扩展 Service Worker 存活，任务结束即清除；不用于安排任何后台任务 |
| 主机访问权限：`<all_urls>` | 让同一个当前页面 Agent 可在用户选择的 HTTP 和 HTTPS 网站上工作，并获取页面引用的资源 |

用户发起 Agent 请求后，只读工具和已知页面操作可以运行；检测到的表单提交每次都需要批准。Runi 不会在后台被动建立浏览历史画像。
