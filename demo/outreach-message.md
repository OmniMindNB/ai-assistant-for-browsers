...
> 嘿，我在做一个浏览器 AI 插件，Aluminum——跟 Gemini in Chrome / Copilot 那种不一样的地方是：
> 改页面前会逐项跟你确认，改完随时能一键撤销，回答问题会引用具体的代码证据而不是瞎猜，模型也是你自己接的。
...
> 你好，我是 [name]，在做一个叫 Aluminum 的浏览器 AI 插件。看你[具体来源：GitHub/公司/社群]
> 经常要处理[第三方页面调试 / QA 测试 / 后台字段配置]这类工作，想请你帮个忙。
>
> 市面上的浏览器 AI（Gemini in Chrome、Copilot、Comet 之类）都锁死在自家模型上，出了问题也不一定能撤销——
> Comet 今年就出过一次真实的 prompt injection 漏洞。Aluminum 不一样：改页面前逐项跟你确认，
> 改完能一键撤销整轮操作，回答问题会点名引用具体的代码/DOM 证据，接的模型也是你自己配置的 API Key。
>
...
> 三个问题，别客气：
> 1. 这 3 天有没有在没人提醒的情况下主动打开过 Aluminum？大概几次，用来干什么？
> 2. 有没有哪次是因为"改完能撤销"或者"改之前会问我"，你才敢让它动手做一个本来不敢让 AI 碰的操作？
> 3. 如果现在把 Aluminum 卸载，你会觉得可惜吗？为什么，或者为什么不？
